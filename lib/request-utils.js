...
export function actionFromReq(req) {
  ...
  if (path === '/sessions/:userId/cookies' && method === 'GET') return 'get_cookies';
  ...
  if (m) {
    const action = m[1];
    if (path.endsWith('/xhr-headers')) return 'get_xhr_headers';
    return action; // navigate, snapshot, click, type, scroll, etc.
  }
